...
Execute the plan with autonomous verification. Claude works through milestones, verifying each one before moving to the next.

## Prerequisites

Before starting build phase, you should have:
- [ ] `feature-spec.md` - Complete feature definition
- [ ] MVP scope clearly defined
- [ ] `technical-design.md` - Architecture and approach
- [ ] `milestones.md` - Testable milestones with success criteria

## The Verification Loop

For each milestone, Claude follows this loop:
...
┌─────────────────────────────────────────┐
│           1. IMPLEMENT                  │
│   Write code for this milestone         │
└─────────────────┬───────────────────────┘
                  │
                  ▼
┌─────────────────────────────────────────┐
│         2. RUN VERIFICATION             │
│   Execute all success criteria checks   │
└─────────────────┬───────────────────────┘
                  │
          ┌───────┴───────┐
          │               │
...
**Steps:**
1. Convert milestones to GitHub issues
2. Create a branch for each issue
3. Work through issues in order
4. Create PR when milestone complete
...
> "Implement this milestone using parallel sub-agents where there are no dependencies between tasks."

This speeds up development significantly.

### Commit After Each Milestone

Don't wait until the end. Commit after each milestone passes verification:
```
feat(plugin): milestone 1 - basic UI shell
...
After significant milestones, update:
- `changelog.md` - What changed
- Architecture docs - If structure evolved
- `CLAUDE.md` - If you learned something important
...
| Situation | Action |
|-----------|--------|
| Starting a milestone | Read success criteria first |
| All tests pass | Commit and move to next milestone |
| Test fails | Debug, fix, re-run (max 3 attempts) |
| Stuck for 3+ attempts | Flag for human, document issue |
| Milestone complete | Update docs, commit |
